...
neteasy_email_manager = EmailMessager(
...
def gpuinfo():
    ...
    for i in range(nvidia_gpu.gpucount):
        ...
        if len(waitlist) == 0:
            ...
            ginfo["currentuser"] = (
                user2name[waitlist[0]["user"]] if waitlist[0]["user"] in user2name else waitlist[0]["user"]
            )
...
def waitlist():
    ...
    for idx, queue in enumerate(gpu_queue_manager.gpu_wait_queues):
        for item in queue:
            ...
            reslist.append(
                {
                    "user": user2name[item["user"]] if item["user"] in user2name else item["user"],
                    "gpuid": idx,
                    "duration": item["duration"],
                    "reason": dbitem["reason"],
                    "container": "无" if item["container"] is None else item["container"],
                    "cmd": "无" if item["cmd"] is None else item["cmd"],
                }
            )
        ...
        for item in queue:
            if item["user"] == uname:
                ...
                if item["start_time"] < datetime.datetime.now():
                    ...
                    r_dict["timeinfo"] = (
                        f"{int(d.total_seconds()//3600)}小时{int((d.total_seconds()%3600)//60)}分钟后结束"
                    )
                else:
                    r_dict["status"] = "排队等待"
                    d = item["start_time"] - datetime.datetime.now()
                    r_dict["timeinfo"] = (
                        f"{int(d.total_seconds()//3600)}小时{int((d.total_seconds()%3600)//60)}分钟后可用"
                    )
                r.append(r_dict)
    return json.dumps(r)
...
def applyforgpu():
    ...
    if container == "选择容器" or cmd == "":  # 没有设置容器和命令
        container = None
        cmd = None
    item: GPURequest = gpu_queue_manager.new_item(
        id, int(attrs["gpuid"]), uname, int(attrs["duration"]), container, cmd
    )
    gpu_request_database.insert(
        {
            "id": id,
            "user": uname,
...
def stopearlyweb():
    ...
    uname = request.cookies.get("uname")
    if (
        len(gpu_queue_manager.gpu_wait_queues[gpuid]) == 0
        or gpu_queue_manager.gpu_wait_queues[gpuid][0]["user"] != uname
    ):
        return "消息过时，请刷新网页！"
...
def imagelist():
    ...
    for image in docker_image_database.all():
        r.append(
            {"imagename": image["show_name"], "env": image["env_description"], "useguide": image["use_description"]}
        )
    return json.dumps(r)
            ...
            r.append(
                {
                    ...
                    "imagename": image_name_dict[container.image],
                    "status": container.status,
                    "ports": "|".join([f"{port['container_port']}->{port['host_port']}" for port in container.ports]),
                }
            )
...
def applyforcontainer():
    ...
    os.system(
        f'echo "{uname} ALL=(ALL:ALL) NOPASSWD: ALL" >> data/user/{uname}/{containername}/sudoers'
    )  # 设置sudo权限
    # 将以上文件的创建者设置为root
    ...
    # 创建容器
    docker_manager.image_instantiation(
        containername,
        uid,
        image,
        "4g",
        volumes,
        f"/home/{uname}/workspace",
        ports_,
        None,
        cmd=f"bash /home/{uname}/start.sh",
    )
    return "容器创建完成."
